...
class MainActivity : ClintActivity(), OverlayHostActivity, SnackbarHostActivity {
    ...
    internal var pendingVpnCountry: com.jhaiian.clint.vpn.VpnCountry? = null
    internal val vpnPermissionLauncher = registerForActivityResult(
        ActivityResultContracts.StartActivityForResult()
    ) { result -> onVpnPermissionResult(result.resultCode == RESULT_OK) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        com.jhaiian.clint.vpn.BrowserVpn.init(this)
        lifecycleScope.launch {
            repeatOnLifecycle(androidx.lifecycle.Lifecycle.State.STARTED) {
                com.jhaiian.clint.vpn.BrowserVpn.state.collect { pushVpnStateToHomePages(it) }
            }
        }
        tabManager.framelessShortcutsEnabled = prefs.getBoolean("shortcut_frameless_enabled", true)
            ...
            ClintComposeTheme(theme = if (uiState.isIncognito) "dark" else startTheme) {
                ...
                if (splashVisible) {
                    com.jhaiian.clint.ui.brand.AetherNetSplash(
                        isDark = androidx.compose.material3.MaterialTheme.colorScheme.background.luminance() < 0.5f,
        ...
        if (isRefreshLinkMode) {
            ...
            if (downloadId != -1) {
                ...
                openNewTab(isIncognito = false, url = HOME_PAGE_URL)
            }
    ...
    fun onTabClosed(index: Int) {
        ...
        if (tabManager.previews().isEmpty()) {
            openNewTab(false, url = HOME_PAGE_URL)
            return
        }
        val activeTab = tabManager.activeTab
        if (activeTab != null && tabManager.isGhostTab(activeTab)) {
            val idx = tabManager.tabs.indexOfFirst { !tabManager.isGhostTab(it) }
            if (idx != -1) { tabManager.switchTo(idx); attachActiveWebView() } else openNewTab(false, url = HOME_PAGE_URL)
        } else if (wasActive) attachActiveWebView()
        else updateTabCount()
    }
    fun onNewTab() { openNewTab(false) }
    fun onNewIncognitoTab() { openNewTab(true, url = HOME_PAGE_URL) }
    ...
    fun onMenuIncognito() { openNewTab(true, url = HOME_PAGE_URL) }
    fun onMenuShare() {
