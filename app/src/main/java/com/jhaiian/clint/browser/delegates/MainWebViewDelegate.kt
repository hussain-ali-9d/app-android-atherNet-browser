...
internal fun MainActivity.createWebView(isIncognito: Boolean): WebView {
    val webView = WebView(if (isIncognito) android.view.ContextThemeWrapper(this, R.style.Theme_AetherNet_IncognitoWeb) else this)
    ...
    applyWebDarkMode(webView, forceDark = isIncognito)
...
internal fun MainActivity.searchBarHint(): String {
    val engineName = when (prefs.getString("search_engine", "duckduckgo")) {
        "brave" -> getString(R.string.engine_brave)
        "ecosia" -> getString(R.string.engine_ecosia)
        "google" -> getString(R.string.engine_google)
        "custom" -> com.jhaiian.clint.browser.customSearchEngineName(prefs).ifBlank { getString(R.string.engine_custom) }
        else -> getString(R.string.engine_duckduckgo)
    }
    return getString(R.string.search_bar_hint, engineName)
}
